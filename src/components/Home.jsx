...
const experiences = [
  {
    role: 'Game Designer Intern', company: 'LUMAS CORP | Bangkok', date: 'Jan - May 2026', tone: 'primary',
  ...
  return (
    <article className="liquid-card project-card group">
      <div className="project-image">
        ...
        <span className="project-type"><i className="fas fa-code mr-1" />{project.type}</span>
      </div>
      <div className="flex flex-1 flex-col p-6">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-2xl font-bold text-slate-900 transition-colors group-hover:text-indigo-600"><a href={project.url} target="_blank" rel="noreferrer">{project.title}</a></h3>
          <span className="date-pill">{project.date}</span>
        </div>
        <p className="mb-4 mt-2 text-sm font-medium text-indigo-600">{project.role}</p>
        <ul className="mb-6 flex-1 space-y-3 text-sm leading-relaxed text-slate-600">
          {project.points.map((point) => <li key={point.label} className="flex gap-2"><i className="fas fa-check check mt-1 text-xs" /><span><strong>{point.label}:</strong> {point.text}</span></li>)}
        </ul>
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <div className="liquid-card mx-auto mt-4 max-w-3xl p-7 text-left md:p-10">
            ...
            <div className="mt-6 flex flex-col gap-3 border-t border-slate-200/70 pt-6 text-sm font-medium text-slate-700 sm:flex-row sm:flex-wrap sm:items-center sm:gap-5">
              <a href="tel:+66611822035" className="contact-link"><i className="fas fa-phone mr-2 text-slate-400" />+66 61 182 2035</a>
              <a href="mailto:witthaya.audomrat@gmail.com" className="contact-link"><i className="fas fa-envelope mr-2 text-slate-400" />witthaya.audomrat@gmail.com</a>
              <div className="flex gap-3"><a className="social-link" href="https://canva.link/5i3s57280gtqrp8" target="_blank" rel="noreferrer" aria-label="CV"><i className="fas fa-file-lines" /></a><a className="social-link" href="https://url.in.th/AsLhl" target="_blank" rel="noreferrer" aria-label="LinkedIn"><i className="fab fa-linkedin-in" /></a><a className="social-link" href="https://github.com/mona65dp" target="_blank" rel="noreferrer" aria-label="GitHub"><i className="fab fa-github" /></a><a className="social-link" href="https://moonun.itch.io" target="_blank" rel="noreferrer" aria-label="Itch.io"><i className="fab fa-itch-io" /></a></div>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-8 md:grid-cols-2">
            <div className="flex flex-col gap-8">
              <div className="liquid-card p-6 md:p-8"><SectionTitle>Education</SectionTitle><h3 className="text-xl font-bold text-slate-900">Dhurakij Pundit University</h3><p className="mt-1 text-sm font-medium text-slate-600">College of Creative Design and Entertainment Technology</p><p className="mt-1 text-sm text-slate-500">Interactive Design and Game Development</p><div className="mt-5 flex items-center justify-between border-t border-slate-200/70 pt-5"><span className="tag tag-accent">GPAX: 3.38</span><span className="date-pill">2022 - 2026</span></div></div>
              <div className="liquid-card p-6 md:p-8"><SectionTitle>Technical Skills</SectionTitle><div className="space-y-5">{Object.entries(skills).map(([group, items]) => <div key={group}><h3 className="skill-heading">{group}</h3><div className="flex flex-wrap gap-2">{items.map((skill) => <span className="tag" key={skill}>{skill}</span>)}</div></div>)}</div></div>
            </div>
            <div className="liquid-card p-6 md:p-8"><SectionTitle>Work Experience</SectionTitle><div className="space-y-8">{experiences.map((experience) => <article className={`timeline-item ${experience.tone}`} key={experience.role}><div className="flex flex-col justify-between gap-2 sm:flex-row"><div><h3 className="text-xl font-bold text-slate-900">{experience.role}</h3><p className="mt-1 font-medium text-indigo-600">{experience.company}</p></div><span className="date-pill self-start">{experience.date}</span></div><ul className="mt-4 list-disc space-y-2 pl-5 text-sm leading-relaxed text-slate-600">{experience.points.map((point) => <li key={point.label}><strong>{point.label}:</strong> {point.text}</li>)}</ul></article>)}</div></div>
          </div>
          <div className="liquid-card mt-8 flex flex-col gap-4 p-6 md:flex-row md:items-center md:p-8"><div className="award-mark"><i className="fas fa-trophy" /></div><div className="flex-1"><h3 className="text-xl font-bold text-slate-900">Achievement <span className="award-label">Highlight</span></h3><p className="font-semibold text-slate-800">Nara Fight (Roblox Game) | Outstanding CWIE Innovative Project Award</p><p className="mt-1 text-sm text-slate-500">Game Designer (Co-op Internship at LUMAS CORP)</p></div><p className="border-t border-slate-200/70 pt-4 text-sm italic text-slate-600 md:w-1/3 md:border-l md:border-t-0 md:pl-6 md:pt-0">Awarded at DPU CWIE Day for innovation and practical implementation in the gaming industry.</p></div>
        </div>
      </section>

      <section id="projects" className="section-spacing relative z-10 px-4">
        <div className="mx-auto max-w-6xl"><div className="mb-10 text-center"><SectionTitle>Projects</SectionTitle></div><div className="mb-10 flex justify-center"><div className="liquid-glass flex gap-1 rounded-2xl p-1.5"><button className={`tab-button ${projectType === 'game' ? 'selected' : ''}`} onClick={() => setProjectType('game')} type="button"><i className="fas fa-gamepad mr-2" />Game Projects</button><button className={`tab-button ${projectType === 'web' ? 'selected' : ''}`} onClick={() => setProjectType('web')} type="button"><i className="fas fa-laptop-code mr-2" />Web & App Projects</button></div></div><div className="grid gap-8 md:grid-cols-2">{projects[projectType].map((project) => <ProjectCard key={project.title} project={project} />)}</div></div>
      </section>
    </main>
  )
}
